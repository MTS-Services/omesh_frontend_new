...
const SuspendedEventsView = () => {
  ...
  const query = useMemo(
    ...
    [dateFilter, page, search, sortQuery]
  );

  const { events, total, currentLimit, loading, error, actionLoading, deleteEvent } =
  ...
  const handleConfirmDelete = async (eventId) => {
    try {
      ...
      // Keep modal open so user can retry if delete fails.
    }
  };
    ...
    <div className="mt-4 rounded-lg border border-gray-100 bg-white shadow-sm md:mt-6">
      <div className="border-b border-gray-100 px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-base font-bold text-gray-900">Listings Suspended Event</h2>
          <FilterTableData
            search={search}
                  ...
                  <td className="flex items-center justify-between px-4 py-2.5 sm:table-cell sm:px-6 sm:py-4">
                    ...
                    <div className="flex items-center gap-3">
                      <Link
                        ...
                        state={{ from: '/dash/events/suspended' }}
...
export default SuspendedEventsView;
